...
def get_cal_poly_resp(intent):
    ...
    session_attributes = {}
    speech_output = "Sorry, I don't understand what you mean"
    reprompt_text = "You never responded to the first test message. Sending another one."
    should_end_session = False
    print(intent)
    slots = intent['slots']
    ...
    max_words = 0
    max_sent = None
    for qr, val in qrs.items():
        wrds = qr.lower().split(' ')
        num_same = len(set(wrds).intersection(words))
        if num_same > max_words:
            max_words = num_same
            max_sent = qr
...
def on_intent(intent_request, session):
    ...
    if intent_name == "cal_poly_parse":
        return get_cal_poly_resp(intent)
